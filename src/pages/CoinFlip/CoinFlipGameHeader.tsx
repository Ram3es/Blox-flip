...
import { PlayerProps } from './CoinFlipGamePlayer'

const CoinFlipGameHeader = forwardRef<HTMLDivElement, PlayerProps>(({ opponent }, ref) => {
  ...
  return (
    <div className='px-4 flex flex-col xs:flex-row justify-between items-center space-y-2 xs:space-y-0 xs:space-x-4 pb-4 xs:pr-10'>
      ...
      <div className='sm:w-96 flex justify-center items-center'>
        ...
        <AvatarWithUsername
          ref={ref}
          username={!currentGame?.joining ? '...' : currentGame?.joining.name}
          avatar={!currentGame?.joining ? QuestionMark : currentGame?.joining.avatar}
        />
      </div>
    </div>
  )
