...
import { IAffiliateData } from '../../types/Affiliates'
import ReferralCode from './ReferralCode'
import { getToast } from '../../helpers/toast'

export const Affiliates = () => {
  const [affiliateData, setAffiliateData] = useState<IAffiliateData | null>(null)
  const { socket } = useSocketCtx()

  useEffect(() => {
    socket.emit('load_affiliates', (err: string | boolean, data: IAffiliateData) => {
  ...
  return (
    <div className="max-w-5xl w-full mx-auto">
        ...
        <div className="px-2 w-full xxs:w-1/2 md:w-auto grow shrink-0 mb-4 flex flex-col">
          ...
          <div className="gradient-blue-secondary flex items-center justify-center py-8 px-5 rounded-lg grow">
            <CoinsWithDiamond
              iconContainerSize="XL"
              iconClasses="w-[18.5px] h-[15.5px]"
              typographyQuantity={affiliateData?.totaldeposited ?? 0}
              typographyFontSize="Size18"
            />
          </div>
        </div>
      ...
      <div className="flex flex-col">
        <AffiliatesForm referalCode={affiliateData?.link} />
        <ReferralCode />
      </div>
      <div className="pb-5 border-b border-blue-highlight mb-6" />
      {affiliateData?.users?.length && <AffiliatesTable users={affiliateData.users} />}
    </div>
  )
}
