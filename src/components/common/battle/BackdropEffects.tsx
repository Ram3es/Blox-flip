...
const BackdropEffects: FC<IBackdropEffects> = ({ statusGame, player, winningCard, isEndGame }) => {
  const getBacdropStars = (image: string) => {
    switch (image) {
      case 'horns': return <img src={IMAGES.redStarBacdrop} alt="gray-backdrop" width="206" height="300" loading="lazy" decoding="async" className=" w-full h-full max-w-full z-10 relative" />
      case 'redCrown': return <img src={IMAGES.redStarBacdrop} alt="gray-backdrop" width="206" height="300" loading="lazy" decoding="async" className=" w-full h-full max-w-full z-10 relative" />
      case 'helmet': return <img src={IMAGES.orangeStarBacdrop} alt="gray-backdrop" width="206" height="300" loading="lazy" decoding="async" className=" w-full h-full max-w-full z-10 relative" />
      default:
  ...
  const renderBackdrop = useMemo(() => winningCard && getBacdropStars(winningCard), [winningCard])
  return (
